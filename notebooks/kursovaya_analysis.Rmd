...
```{r}
load(file = here(data_dir, "data_raw.Rda"))
load(file = here(data_dir, "data_tpm.Rda"))
```

```{r}
data_p53 <- read.csv(here(data_dir, "p53_full.csv"), sep = ";", header = T)
up_p53_mouse <- read.csv(here(data_dir, "up_p53_mouse.csv"), sep = ",", header = T)
down_p53_mouse <- read.csv(here(data_dir, "down_p53_mouse.csv"), sep = ",", header = T)
geni_upr <- read.csv2(here(data_dir, "ana_upr.csv"), header = F)
```

```{r}
load(here(data_dir, "input_for_figures.Rdata"))
```
...
sp <- ggplot(as.data.frame(object$layout), aes(umap_met[, 1], umap_met[, 2], color = Cluster)) +
    geom_point()
...
claculateClusters <- function(geneIndices, deepSplit) {
    ...
    clusterStatistics <- cluster.stats(
        d = my.dist,
        clustering = my.clusters
    )

    result <- list(
        "PearsonGamma" = clusterStatistics$pearsongamma,
...
sp <- ggplot(as.data.frame(object$layout), aes(umap_met[, 1], umap_met[, 2], color = as.factor(batch))) +
    geom_point() +
    labs(x = "Umap1", y = "Umap2", col = "Batch") +
    ggtitle(NULL)
sp
```
...
haCol1 <- HeatmapAnnotation(
    df = data.frame(
        Cluster = numero_clu,
        Condition = medium_finale
    ),
    col = list(
        Condition = c("Cell competition ON" = "yellow 1", "Cell competition OFF" = "green 1"),
        Cluster = c(
...
diffu <- ggplot(tre, aes(x = DC1, y = DC2, color = Cluster)) +
    geom_point()
...
diffu <- ggplot(tre, aes(x = DC1, y = DC2, color = Condition)) +
    geom_point() +
...
diffu <- ggplot(dati, aes(x = prima, y = seconda, color = as.factor(cluster))) +
    geom_point()
...
diffu <- ggplot(dati, aes(x = prima, y = seconda, color = as.factor(Condition))) +
    geom_point()

diffu <- ggplot(dati, aes(x = prima, y = seconda, color = Condition)) +
    geom_point()
...
dfForPlot <- data.frame(
    clusters = as.character(Cluster_clu_fmk),
    exp = time_only_fmk,
...
dfForPlot <- data.frame(
    clusters = as.character(Cluster_clu_dmso),
    exp = time_projec,
...
ggplot(
    ...
    ylab("pseudo-time FMK (from only FMK epiblast)") +
    ggtitle(paste("Spearman correlation :", round(cor(time_all_epi, time_fmk_epi_fmk_epiblasto, method = "spearman"), 3), sep = " "))
```
...
bo <- data.frame(
    Condition = Condition_new, Cluster = numero_clu,
...
singolo_correlazione <- function(a, b, c, d) {
    ...
    p <- ggplot(data = dati, aes(x = exp_1_los_1, y = exp_1_los)) +
        geom_point(col = "#006400") +
        xlab(d) +
        ylab(c) +
        ggtitle(paste(c, "vs", d, "-spearman corr:", round(cor(exp_1_los_1, exp_1_los, method = "spearman"), 3), sep = " "))
    p
}
...
risultato <- apply(data, 1, function(x) {
    sum(x > 15)
})
...
sp <- ggplot(as.data.frame(object$layout), aes(umap_met[, 1], umap_met[, 2], color = Cluster)) +
    ...
    xlab("Umap 1") +
    ylab("Umap 2") +
    ggtitle("Cluster on top (FDR<0.01)  varying genes along pseudotime")
...
# up_p53_mouse=read.csv(here(data_dir, "up_p53_topo.csv"),sep=",",header=T)
# down_p53_mouse=read.csv(here(data_dir, "down_p53_topo.csv"),sep=",",header=T)
# test up
...
pie + blank_theme +
    ...
    ), size = 5) + ggtitle("Targets activated by P53")
...
pie + blank_theme +
    ...
    geom_text(aes(
        y = c(4, 30),
        label = percent(value / 41)
    ), size = 5) + ggtitle("Targets repressed by P53")
```
...
gene_expression <- function(a, b) {
    ...
    if (sum(nome_gene$Gene.stable.ID[which(nome_gene$Gene.name == a)] %in% colnames(gam.fitted)) == 0) {
        titolo <- paste(a, "no in the background", b, sep = " ")
    }
    if (sum((nome_gene$Gene.stable.ID[which(nome_gene$Gene.name == a)] %in% colnames(gam.fitted) > 0) & (nome_gene$Gene.stable.ID[which(nome_gene$Gene.name == a)] %in% row.names(classe_geni) == 0))) {
        titolo <- paste(a, "no up/down", b, sep = " ")
    }
...
gene_expression <- function(a, b) {
    ...
    p <- ggplot(
        ...
        ggtitle(titolo) +
        geom_smooth(method = "loess", se = F, col = "red") +
        scale_color_manual(values = c("#DD6400", "#0000FF", "#006400")) +
        labs(col = "Cluster")
    ...
    p <- ggplot(
        ...
        ylab("log 10 norm counts") +
        ggtitle(titolo) +
        scale_colour_manual(values = c("#DD6400", "#0000FF", "#006400")) +
        labs(colour = "Cluster")
...
```{r}
raw_dati <- read.table(here(data_dir, "GeneCounts.csv"), sep = ",", header = T)
...
meta_data <- read.table(here(data_dir, "metadata_ana_new.csv"), sep = ";", header = F)
...
```{r}
ward_down_no <- read.csv(here(data_dir, "down_ana_last_version.csv"))
...
```{r}
ward_down_no <- read.csv(here(data_dir, "up_ana_last_version.csv"))
# go_up_34_function=go_up_34[go_up_34$source=="GO:MF",]
# up_34_function=as.vector(go_up_34_function$term_name)
# go_up_34_process=go_up_34[go_up_34$source=="GO:BP",]
# up_34_process=as.vector(go_up_34_process$term_name)
...
```{r}
ward_down_no <- read.csv(here(data_dir, "down_vivo_last_version.csv"))
# go_up_34_function=go_up_34[go_up_34$source=="GO:MF",]
# up_34_function=as.vector(go_up_34_function$term_name)
# go_up_34_process=go_up_34[go_up_34$source=="GO:BP",]
# up_34_process=as.vector(go_up_34_process$term_name)
...
```{r}
ward_down_no <- read.csv(here(data_dir, "up_vivo_last_version.csv"))
# go_up_34_function=go_up_34[go_up_34$source=="GO:MF",]
# up_34_function=as.vector(go_up_34_function$term_name)
# go_up_34_process=go_up_34[go_up_34$source=="GO:BP",]
# up_34_process=as.vector(go_up_34_process$term_name)
...
```{r}
# bo=read.csv(here(data_dir, "GSE109071_series_matrix.txt"),sep="\t",header = F)


meta_dati_1 <- read.csv(here(data_dir, "SraRunTable.txt.csv"))
meta_dati <- read.table(here(data_dir, "sample.tsv"), sep = "\t", header = T)
dati_raw <- read.table(here(data_dir, "GSE109071_read.txt"))
age <- meta_dati_1$Age
age <- (as.vector(age))
...
load(here(data_dir, "length_genes.Rda"))
...
# load(here(data_dir, "HVGC_giusto_no_un.Rda")
# load(here(data_dir, "dissimilarity_all.Rda")
...
diffu <- ggplot(dati_plot, aes(x = prima, y = seconda, color = (colore_plot))) +
    geom_point() +
    labs(x = "DC1", y = "DC2") +
    ggtitle("Diffusion Map") +
    labs(col = "Cluster") +
...
```{r}
dfForPlot <- data.frame(
    clusters_fmk = as.character(c(clusters, clusters_pro_fmk)),
...
ggplot(
    ...
    xlab(NULL) +
    geom_sina() +
    ggtitle("Pseudo time FMK cells (274) and epiblast cells E5.5, E6.25 and E6.5 (520)") +
    ylim(c(0, max(time_dati_nuovi))) +
    theme(
        axis.title.x = element_blank(),
...
```{r}
dfForPlot <- data.frame(
    clusters_tutto = as.character(c(clusters, clusters_pro_fmk, clusters_pro_dmso)),
...
ggplot(
    ...
    labs(col = "Cluster") +
    geom_sina() +
    ggtitle("Pseudo time CI (274), DMSO (238) and epiblast cells E5.5, E6.25 and E6.5 (520)") +
    ylim(c(0, max(time_dati_nuovi))) +
    theme(
        axis.title.x = element_blank(),
        axis.text.x = element_blank(),
...
```{r}
load(here(data_dir, "meta_dati.Rda"))
nomi <- read.csv(here(data_dir, "old_2_cell_names.csv"))
nomi <- nomi$X0
nomi <- as.vector(nomi)

all_pos <- read.csv(here(data_dir, "raw_counts_allele_mt.csv"))
all_pos <- all_pos[, -1]
...
load(here(data_dir, "provo_ward_no_un.Rda"))
load(here(data_dir, "name_full.Rda")) # name of the old bam file
# load(here(data_dir, "nomi_nuovi.Rda"))#name on array express
# load(here(data_dir, "nomi_vecchi.Rda"))#
...
entropy_method <- function(all_pos, cell_stage, vedere_all, vedere_numb, number_reads, number_positions, filtering = 1, selection = FALSE, cell_clu_1 = NULL, cell_clu_3 = NULL, cell_clu_4 = NULL) {
    ...
    pos_cov <- apply(somma, 1, function(x) {
        sum(x > number_reads)
    })

    dfForPlot <- data.frame(
        clusters = row.names(somma),
    ...
    if (filtering == 1) {
        all_pos_update <- apply(somma_qc, 2, function(x) {
            if (sum(x > number_reads) == length(row.names(somma_qc))) {
                return(TRUE)
            } else {
                return(FALSE)
            }
    ...
    if (filtering == 2) {
        all_pos_update <- apply(somma_qc, 2, function(x) {
            ...
            if ((sum(clu_1 > number_reads) > 0.5 * length(clu_1)) & (sum(clu_3 > number_reads) > 0.5 * length(clu_3)) & (sum(clu_4 > number_reads) > 0.5 * length(clu_4))) {
                return(TRUE)
            } else {
                return(FALSE)
            }
    ...
    if (selection == TRUE) {
        test_cell <- apply(somma_qc_qc, 1, function(x) {
            sum(x > number_reads)
        })
...
plot_heteroplasmy_cell_competition <- function(position, entropia_matrix, cluster, indici) {
    ...
    ggplot(
        data = dati_scatter_1,
        aes(
            x = factor(cluster, levels = c("Winner Epiblast", "Intermediate", "Loser Epiblast"), ordered = TRUE),
            y = mutazione,
        ...
        labs(y = "Heteroplasmy") +
        ggtitle(position) +
        scale_color_manual(values = c("#DD6400", "#0000FF", "#006400"), labels = c("Winner Epiblast", "Intermediate", "Loser Epiblast"))
}
```
...
plot_allele_frequency_cell_competition <- function(position, entropia_matrix, allele_matrix, cluster, indici, number) {
    ...
    plot_1 <- ggplot(
        ...
        ylim(0, 1) +
        scale_color_manual(values = c("#DD6400", "#0000FF", "#006400"), labels = c("Winner Epiblast", "Intermediate", "Loser Epiblast")) +
        theme(axis.text = element_text(size = number))
...
plot_losing_score <- function(position, entropia_matrix, cluster, time, fit, indici) {
    ...
    plot <- ggplot(dati_scatter_1, aes(x = time, y = mutation)) +
        geom_point(aes(colour = (cluster))) +
        labs(col = "Cluster") +
        labs(x = "Losing Score") +
        labs(y = "Heteroplasmy") +
        scale_color_manual(values = c("#DD6400", "#0000FF", "#006400"), labels = c("Winning Epiblast", "Intermediate", "Losing Epiblast")) +
        ggtitle(paste(as.character(position), "adjusted p value=", round(fit[position, 1], (floor(-log10(fit[position, 1]))) + 1), sep = " ")) +
...
plot_losing_score_average <- function(average, cluster, time, indici, p_value) {
    ...
    plot <- ggplot(dati_scatter_1, aes(x = time, y = mutation)) +
        ...
        labs(y = "Heteroplasmy") +
        scale_color_manual(values = c("#DD6400", "#0000FF", "#006400"), labels = c("Winning Epiblast", "Intermediate", "Losing Epiblast")) +
        ggtitle(paste("Average", p_value, sep = " ")) +
        geom_smooth(method = "loess", se = F, color = "black", fullrange = F)
    plot
}
```
...
```{r}
mt_name <- read.table(here(data_dir, "useful_mt_names.txt"), sep = "\t", header = T)
...
# load(here(data_dir, "time_fmk_epi.Rda"))
# load(here(data_dir, "cells_epi_fmk.Rda"))
...
identification_cluster <- function(a) {
    ...
    p <- ggplot(dati_cluster, aes(x = Cluster, y = frequenze_norm, fill = Cluster)) +
        geom_bar(stat = "identity") +
        scale_fill_manual(values = c("#DD6400", "#0000FF", "#006400")) +
        xlab(NULL) +
        ylab("Percentage of cells ") +
        ggtitle("Percentage of cells in cluster carrying significant mutations in mt-Rnr1")
    p
}
